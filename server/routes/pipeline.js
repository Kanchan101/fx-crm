...
router.patch('/:id/status', authenticate, async (req, res) => {
  try {
    ...
    if (status === 'On Hold' && hold_reason) { updateSql += `, hold_reason=$${paramIdx++}`; updateParams.push(hold_reason); }
    if (status === 'Interview' && interview_round) { updateSql += `, interview_round=$${paramIdx++}`; updateParams.push(interview_round); }
    if (!['Rejected','On Hold','Dropped','Joined'].includes(status)) { updateSql += ', reject_reason=NULL, drop_reason=NULL, hold_reason=NULL'; }
