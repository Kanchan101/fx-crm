  ...
  try {
    ...
    try {
      const intResult = await query(
        `SELECT i.*, ca.name as candidate_name, ca.email as candidate_email, ca.phone as candidate_phone
         FROM interviews i JOIN candidates ca ON ca.id = i.candidate_id
         WHERE i.job_id = $1 ORDER BY i.interview_date ASC, i.interview_time ASC`, [req.params.id]
      );
      interviews = intResult.rows;
    } catch(e) {}

    res.json({ requirement: result.rows[0], assigned_team: assignments.rows, pipeline: pipeline.rows, spocs, interviews });
...
// PATCH status — NEW WORKFLOW with hold_reason support
router.patch('/:id/pipeline/:pipelineId/status', authenticate, async (req, res) => {
  try {
    ...
    if (status === 'On Hold' && hold_reason) {
      updateSql += `, hold_reason=$${paramIdx++}`;
      updateParams.push(hold_reason);
    }
    if (status === 'Interview' && interview_round) {
      updateSql += `, interview_round=$${paramIdx++}`;
      updateParams.push(interview_round);
    }
    // Clear reasons when moving to non-exit status
    if (!CLOSED_STATUSES.includes(status) && status !== 'Joined') {
      updateSql += ', reject_reason=NULL, drop_reason=NULL, hold_reason=NULL';
    }
    ...
    await query('INSERT INTO activity_log (user_id,action,entity_type,entity_id,details) VALUES ($1,$2,$3,$4,$5)',
      [req.user.id, 'STATUS_CHANGE', 'pipeline', req.params.pipelineId, JSON.stringify({ from: old, to: status, reject_reason, drop_reason, hold_reason })]);
